 ...
 * Includes document-specific vague qualifiers for backwards compatibility
 */
...
export function getSlopPenalty(text) {
  ...
  // Thresholds calibrated for documents where precision is critical
  let penalty = 0;
  const issues = [];

  // Count the most problematic patterns for documents
  ...
  if (slopResult.score >= 40 || patternCount >= 10) {
    ...
    penalty = 4; // Moderate slop - any 3+ buzzwords/vague terms is problematic
