...
test.describe('Validator (project-attached mode)', () => {
  test('loads project Phase 3 markdown from IndexedDB via query string (no paste)', async ({
    page,
  }) => {
  ...
  test('persists draft edits to project-scoped validator state (survives reload)', async ({
    page,
  }) => {
    await page.goto('/assistant/?type=one-pager');
    await page.waitForLoadState('networkidle');
  ...
  test('Validator Save persists to project so Assistant Phase 3 reflects latest saved content', async ({
    page,
  }) => {
    await page.goto('/assistant/?type=one-pager');
    await page.waitForLoadState('networkidle');
  ...
  for (const doc of DOC_TYPES) {
    test(`Assistant Save Response then Tune & Refine opens Validator with latest Phase 3 content (${doc.type}, legacy flat field)`, async ({
      page,
    }) => {
      ...
      const [popup] = await Promise.all([page.waitForEvent('popup'), page.click('#validate-btn')]);
      await popup.waitForLoadState('networkidle');
      await expect(popup.locator('#attached-badge')).toBeVisible();
      await expect(popup.locator('#editor')).toHaveValue(updated);
    });

    test(`Assistant Save Response then Tune & Refine opens Validator with latest Phase 3 content (${doc.type}, nested-only project)`, async ({
      page,
    }) => {
      await page.goto(`/assistant/?type=${doc.type}`);
      await page.waitForLoadState('networkidle');
    ...
    test(`Assistant Save Response then Copy Final + Tune & Refine always use latest Phase 3 content even with existing validator draft (${doc.type})`, async ({
      page,
    }) => {
      ...
      await expect(page.locator('#toast-container')).toContainText('Final document copied', {
        timeout: 10000,
      });
  ...
  test('shows an attached-mode error state when phase output is empty', async ({ page }) => {
    ...
    await expect(page.locator('#attached-open-assistant')).toHaveAttribute(
      'href',
      `/assistant/?type=one-pager#project/${projectId}`
    );
  });

  test('can draft from an empty phase output and apply back to the Assistant project', async ({
    page,
  }) => {
    await page.goto('/assistant/?type=one-pager');
    await page.waitForLoadState('networkidle');
