...
|----|-------------|---------|------|--------------|--------------|
| FR1 | Dashboard displays session duration metrics | P1 | 🔄 Two-Way | Given date range, When loaded, Then show avg/p50/p95 in <3s | Given invalid range, Then show "Invalid date range" error |
| FR2 | Dashboard displays drop-off funnel | P1 | 🔄 Two-Way | Given funnel selected, When loaded, Then show conversion % per step | Given no data, Then show "No sessions in range" message |
| FR3 | User can export data to CSV | P1 | 🔄 Two-Way | Given data displayed, When export clicked, Then download CSV in <5s | Given >100K rows, Then show "Export limited to 100K rows" |
...
| PMs misinterpret metrics | Medium | Medium | Add tooltips with metric definitions | Office hours training session |
| Data accuracy questioned | Low | High | Validate against known engineering queries before launch | Provide "report discrepancy" feedback channel |

## 15. Open Questions

1. Which 3 funnels should be pre-configured for drop-off analysis? (Need PM input)
2. Should we show user-level data or only aggregates? (Privacy review needed)
