...
export const REQUIRED_SECTIONS = [
  ...
  {
    pattern: /^(#+\s*)?(problem|challenge|issue|opportunity|context|pain.?point|current.?pain)/im,
    name: 'Problem Statement',
    weight: 2,
  },
  // Solution - matches Phase1 "Present Proposed Solutions"
  {
    pattern: /^(#+\s*)?(solution|proposal|approach|recommendation|strategy)/im,
  ...
  {
    pattern:
      /^(#+\s*)?(impact|benefit|outcome|value|roi|return|financial.?impact|gross.?profit|revenue)/im,
    name: 'Business Impact',
    weight: 2,
  },
  // Implementation - matches Phase1 "Timeline" subsection
  {
    pattern: /^(#+\s*)?(implementation|plan|timeline|roadmap|execution|next.?steps)/im,
  ...
  {
    pattern:
      /^(#+\s*)?(resource|budget|cost|investment|team|pricing|price|subscription|commercials)/im,
    name: 'Resources/Budget',
    weight: 1,
  },
  // Risks - optional but scored
  {
    pattern: /^(#+\s*)?(risk|assumption|dependency|constraint)/im,
    name: 'Risks/Assumptions',
    weight: 1,
  },
  // Success Metrics - optional but scored
  {
    pattern: /^(#+\s*)?(success|metric|kpi|measure|objective)/im,
...
export const PROBLEM_PATTERNS = {
  ...
  problemLanguage:
    /\b(problem|challenge|issue|opportunity|gap|limitation|constraint|blocker|barrier|pain.?point)\b/gi,
  urgency:
    /\b(urgent|critical|immediate|priority|time.sensitive|deadline|window|opportunity.cost)\b/gi,
  quantified:
...
export const SOLUTION_PATTERNS = {
  ...
  actionable:
    /\b(implement|execute|deliver|launch|build|create|develop|establish|deploy|rollout)\b/gi,
...
export const IMPACT_PATTERNS = {
  impactSection:
    /^(#+\s*)?(impact|benefit|outcome|value|roi|return|business.case|financial.?impact)/im,
  impactLanguage: /\b(impact|benefit|value|roi|return|outcome|result|improvement|gain|savings)\b/gi,
  quantified:
    /\d+\s*(%|million|thousand|hour|day|week|month|year|\$|dollar|user|customer|revenue)/gi,
  financialTerms:
    /\b(revenue|cost|savings|profit|margin|efficiency|productivity|reduction|increase)\b/gi,
  ...
  dealershipImpact:
    /\b(gross.?profit.*store|per.?store|per.?rooftop|call.?conversion|appointment.?rate|inbound.?call|outbound.?connection|missed.?opportunit|vendor.?switch)\b/gi,
};
...
export const IMPLEMENTATION_PATTERNS = {
  ...
  datePatterns:
    /\b(week|month|quarter|q[1-4]|year|fy\d+|\d{4}|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/gi,
...
export const STAKEHOLDER_PATTERNS = {
  ...
  stakeholderConcerns:
    /\b(finance|fp&a|fp.?&.?a|financial.planning|hr|people.?team|people.?ops|legal|compliance|equity|liability|approval|sign.?off|cfo|cto|ceo|vp|director|gm|general.?manager)\b/gi,
