...
export const EVIDENCE_PATTERNS = {
  ...
  quantified:
    /\d+\s*(%|million|thousand|hour|day|week|month|year|\$|dollar|user|customer|transaction|k\b|m\b)/gi,
  businessFocus: /\b(business|customer|user|market|revenue|profit|competitive|strategic|value)\b/gi,
  sources:
    /\b(gartner|forrester|mckinsey|dora|radford|idc|according.to|research|study|survey|benchmark|internal.data)\b/gi,
  beforeAfter:
    /\b(before|after|from.+to|baseline|current|target|improvement|reduction|increase)\b/gi,
};
...
export const FINANCIAL_PATTERNS = {
  ...
  roiCalculation:
    /\b(roi|return.on.investment|benefit.?.cost|cost.?.benefit|net.present.value|npv)\b/gi,
  roiFormula:
    /(\d+\s*[-−–]\s*\d+)\s*[/÷]\s*\d+|roi\s*[=:]\s*\d+|\(.*benefit.*[-−–].*cost.*\)\s*[/÷]|savings\s*[/÷]\s*investment|\$[\d,]+\s*[/÷]\s*\$[\d,]+|\([^)]+[-−–][^)]+\)\s*[/÷]\s*\S+/gi,
  ...
  tcoAnalysis:
    /\b(tco|total.cost.of.ownership|3.?year|three.?year|implementation.cost|training.cost|operational.cost|opportunity.cost|hidden.cost)\b/gi,
...
export const OPTIONS_PATTERNS = {
  ...
  doNothing:
    /\b(do.?nothing|status.?quo|no.?action|inaction|if.we.don't|without.this|current.path|option.?a)\b/gi,
  alternatives:
    /\b(alternative|option|approach|scenario|build.vs.buy|buy.vs.build|option.?[abc123]|path.?[abc123])\b/gi,
  recommendation:
    /\b(recommend|recommendation|proposed|chosen|selected|preferred|our.choice|we.propose)\b/gi,
  comparison:
    /\b(compare|comparison|versus|vs\.?|trade.?off|pros?.and.cons?|advantage|disadvantage)\b/gi,
  minimalInvestment: /\b(minimal|minimum|low.?cost|basic|mvp|phase.?1|incremental)\b/gi,
  fullInvestment:
    /\b(full.?investment|full.?option|strategic.?transformation|target.?state|recommended.?approach|option.?c|comprehensive|enterprise.?solution)\b/gi,
};
