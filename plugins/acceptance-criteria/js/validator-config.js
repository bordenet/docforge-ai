...
export const CLARITY_PATTERNS = {
  ...
  actionVerbs:
    /\b(implement|create|build|render|handle|display|show|hide|enable|disable|validate|submit|load|save|delete|update|fetch|send|receive|trigger|navigate|redirect|authenticate|authorize)\b/gi,
  // Measurable metrics with units - expanded to include common technical units
  metricsPattern:
    /(?:≤|≥|<|>|=|under|within|less than|more than|at least|at most)?\s*\d+(?:\.\d+)?\s*(ms|milliseconds?|seconds?|s|%|percent|kb|mb|gb|tb|px|items?|users?|requests?|errors?|days?|hours?|minutes?|calls?|connections?|records?|retries?|attempts?|rows?|entries?|results?|pages?|clicks?|taps?|events?)/gi,
  // Specific thresholds
  thresholdPattern:
    /\b(exactly|at least|at most|maximum|minimum|up to|no more than|no less than)\s+\d+/gi,
};
...
export const TESTABILITY_PATTERNS = {
  ...
  vagueTerms:
    /\b(works?\s+correctly|handles?\s+properly|appropriate(ly)?|intuitive(ly)?|user[- ]friendly|seamless(ly)?|fast|slow|good|bad|nice|better|worse|adequate(ly)?|sufficient(ly)?|reasonable|reasonably|acceptable|properly|correctly|as\s+expected|as\s+needed)\b/gi,
  ...
  implementationPattern:
    /\b(postgres(?:ql)?|mysql|mongodb|redis|sql|react|vue|angular|svelte|tailwind|css|scss|sass|aws|lambda|s3|ec2|gcp|azure|docker|kubernetes|k8s|api\s+endpoint|microservice|graphql|rest\s+api|webpack|vite|npm|yarn)\b/gi,
};
...
export const COMPLETENESS_PATTERNS = {
  ...
  errorCasePattern:
    /\b(error|fail|invalid|empty|null|undefined|missing|timeout|offline|denied|unauthorized|forbidden|not found|exception)\b/gi,
  // Edge case indicators - tightened to avoid false positives
  edgeCasePattern:
    /\b(edge\s+case|boundary\s+condition|boundary\s+value|upper\s+limit|lower\s+limit|maximum\s+value|minimum\s+value|empty\s+state|no\s+results|only\s+one|zero\s+items?|overflow|underflow|race\s+condition|concurrent|simultaneous)\b/gi,
