/**
 * PRD Validator Secondary Detection Functions
 * Value proposition, user personas, problem statement, and NFRs
 */

import { VALUE_PROPOSITION_PATTERNS } from './validator-config.js';
...
export function detectUserPersonas(text) {
  ...
  const hasPersonaSection =
    /^#+\s*(\d+\.?\d*\.?\s*)?(user\s*persona|personas?|target\s+user|audience|customer\s+profile|primary\s+user)/im.test(
      text
    );
  ...
  const userTypes =
    text.match(
      /\b(admin|administrator|end.?user|power.?user|developer|manager|customer|stakeholder|buyer|seller|operator|analyst|engineer|designer|user|professional|owner|team\s+lead|solo\s+developer)\b/gi
    ) || [];
  const uniqueUserTypes = [...new Set(userTypes.map((u) => u.toLowerCase().trim()))];

  const hasPainPoints =
    /\b(pain.?point|problem|challenge|frustrat|struggle|difficult|issue|context.?switch|cognitive\s+overhead|loses?\s+track|scattered|disorganized)\b/i.test(
      text
    );
  if (hasPainPoints) personaIndicators.push('Pain points addressed');

  const hasScenarios =
    /\b(scenario|use.?case|user.?journey|workflow|user.?flow|daily\s+routine|typical\s+day)\b/i.test(
      text
    );
  if (hasScenarios) personaIndicators.push('User scenarios described');

  const hasPersonaDepth =
    /\*\*primary\s+user\*\*:|\*\*target\s+user\*\*:|who\s+(will|would)\s+use|target\s+audience\s+is/i.test(
      text
    );
...
export function detectProblemStatement(text) {
  ...
  const hasProblemSection =
    /^#+\s*(\d+\.?\d*\.?\s*)?(problem|goal|objective|why|motivation|current\s+state|target\s+state)/im.test(
      text
    );
  if (hasProblemSection) indicators.push('Dedicated problem statement section');

  const hasProblemLanguage = /\b(problem|challenge|current.?state|today|existing|pain)\b/i.test(
    text
  );
  if (hasProblemLanguage) indicators.push('Problem framing language');

  const hasValueProp =
    /\b(value|benefit|outcome|result|enable|empower|improve|reduce|increase|streamline|automate|simplify|eliminate|save|prevent|accelerate|enhance|optimize|transform|deliver|provide|ensure|achieve|solution|unified|integrated|seamless|efficient)\b/i.test(
      text
    );
  if (hasValueProp) indicators.push('Value proposition language');

  const hasWhyExplanation = /\b(so\s+that|in\s+order\s+to|because|this\s+will|enabling)\b/i.test(
    text
  );
...
export function detectNonFunctionalRequirements(text) {
  ...
  return {
    ...
    hasNFRSection:
      /^(?:#+\s*)?(\d+\.?\d*\.?\s*)?(non.?functional|quality\s+attribute|nfr|performance|security|technical\s+requirement)/im.test(
        text
      ),
  };
}
