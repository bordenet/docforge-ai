...
export const REQUIRED_SECTIONS = [
  ...
  {
    pattern: /^(#+\s*)?(consequence|impact|result|outcome|implication)/im,
    name: 'Consequences',
    weight: 2,
  },
...
export const CONTEXT_PATTERNS = {
  ...
  contextLanguage:
    /\b(context|background|problem|situation|challenge|need|requirement|constraint|driver|force)\b/gi,
  constraints: /\b(constraint|limitation|requirement|must|should|cannot|restriction|boundary)\b/gi,
  quantified:
    /\d+\s*(%|million|thousand|hour|day|week|month|year|\$|dollar|user|customer|transaction)/gi,
  businessFocus:
    /\b(business|customer|user|market|revenue|profit|competitive|strategic|value|stakeholder)\b/gi,
};
...
export const VAGUE_DECISION_PATTERNS =
  /\b(strategic\s+approach|architectural\s+intervention|improve\s+scalability|more\s+maintainable|better\s+architecture|enhance\s+performance|optimize\s+the\s+system|modernize\s+the\s+platform|transform\s+the\s+infrastructure)\b/gi;
...
export const OPTIONS_PATTERNS = {
  ...
  comparison:
    /\b(compare|versus|vs|pro|con|advantage|disadvantage|trade.?off|benefit|drawback)\b/gi,
...
export const CONSEQUENCES_PATTERNS = {
  ...
  positive:
    /\b(benefit|advantage|improve|enable|allow|simplify|reduce|faster|easier|better|scalable|maintainable|testable|decoupled|independent|automated)\b/gi,
  negative:
    /\b(drawback|disadvantage|risk|cost|slower|harder|worse|trade.?off|latency|coupling|dependency|bottleneck|single.point.of.failure|migration.effort)\b/gi,
  neutral: /\b(change|require|need|must|will.need|migration|update)\b/gi,
};

// Vague consequence terms
export const VAGUE_CONSEQUENCE_TERMS =
  /\b(complexity|overhead|difficult|challenging|problematic|issues?|concerns?)\b/gi;

// Status patterns
export const STATUS_PATTERNS = {
  statusSection: /^(#+\s*)?(status|state)/im,
  statusValues:
    /\b(proposed|accepted|deprecated|superseded|rejected|draft|approved|implemented)\b/gi,
  datePatterns:
    /\b(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{2,4}|january|february|march|april|may|june|july|august|september|october|november|december)\b/gi,
...
export const RATIONALE_PATTERNS = {
  ...
  rationaleLanguage:
    /\b(because|reason|rationale|justification|why|due.to|since|therefore|thus)\b/gi,
...
export const TEAM_PATTERNS =
  /training.*need|skill gap|hiring impact|team ramp|learning curve|expertise required|onboarding|team structure|hiring|staffing/i;

// Subsequent ADR pattern
export const SUBSEQUENT_PATTERN =
  /subsequent ADR|follow-on ADR|triggers ADR|future ADR|ADR-\d+|triggers.*(?:decision|choice).*(?:on|for|about|regarding)\s+\w+/i;

// Review timing pattern
export const REVIEW_PATTERN =
  /\b\d+\s*(days?|weeks?|months?)\s*(review|reassess|revisit)|after-action|review.*timing|recommended.*review|review in \d+|quarterly\s+review|annual\s+review/i;

// Alternatives comparison pattern
export const ALTERNATIVES_PATTERN =
  /we considered .+?,\s*.+?(?:,\s*.+?)?\s*(?:and\s+.+?\s+)?but (?:chose|selected|decided|went with)/i;
...
export const CONFIRMATION_PATTERNS = {
  ...
  validationLanguage:
    /\b(confirm|validate|verify|review|test|audit|check|compliance|DCAR|architecture review|code review|load test)\b/gi,
...
export const Y_STATEMENT_PATTERNS = {
  ...
  altFormats:
    /(?:we\s+(?:chose|selected|decided\s+on)|selected[:\s]+)["']?([^"'\n,]+)["']?,?\s*because/i,
  // Y-statement justification pattern
  justification:
    /because\s+(?:it\s+)?(?:is\s+the\s+only\s+option|meets?\s+|resolves?\s+|comes?\s+out\s+best|provides?\s+|enables?\s+|allows?\s+)/i,
};
...
export const MORE_INFO_PATTERNS = {
  sectionHeader:
    /^#+\s*(?:more\s+information|additional\s+information|references|links|evidence)\b/im,
  hasLinks: /\[.+\]\(.+\)/g,
  hasEvidence:
    /\b(study|research|benchmark|test|experiment|data|survey|interview|usability\s+test)\b/gi,
};
...
export const TECHNICAL_CONTEXT_PATTERNS = {
  ...
  diagramReference:
    /\b(?:diagram|figure|flowchart|sequence|architecture\s+diagram|mermaid|plantuml)\b/gi,
  techStackLanguage:
    /\b(?:tech\s+stack|technology\s+stack|framework|library|platform|infrastructure)\b/gi,
...
export const REVERSIBILITY_PATTERNS = {
  ...
  oneWayDoor:
    /\b(?:one[-\s]?way\s+door|irreversible|permanent|hard\s+to\s+undo|point\s+of\s+no\s+return)\b/gi,
  twoWayDoor:
    /\b(?:two[-\s]?way\s+door|reversible|can\s+be\s+undone|easy\s+to\s+revert|rollback)\b/gi,
...
export const SCOPE_IMPACT_PATTERNS = {
  ...
  systemsAffected:
    /\b(?:system|service|component|module|api|database)s?\s+(?:affected|impacted)\b/gi,
};
...
export const ALTERNATIVES_DEPTH_PATTERNS = {
  alternativesSection:
    /^#+\s*(?:alternatives?\s+considered|options?\s+(?:evaluated|considered)|other\s+options?)\b/im,
...
export const CHANGELOG_PATTERNS = {
  changelogSection:
    /^#+\s*(?:changelog|change\s+log|history|revision\s+history|version\s+history)\b/im,
...
export const ASR_PATTERNS = {
  asrSection:
    /^#+\s*(?:ASR|architecture\s+significant|significant\s+requirements?|key\s+requirements?)\b/im,
  asrLanguage:
    /\b(?:architecturally\s+significant|architecture\s+decision|critical\s+requirement|key\s+driver)\b/gi,
...
export const COST_PATTERNS = {
  ...
  effortLanguage:
    /\b(?:person[-\s]?days?|person[-\s]?weeks?|person[-\s]?months?|story\s+points?|sprint|FTE|headcount)\b/gi,
...
export const TIMELINE_PATTERNS = {
  ...
  deadlineLanguage:
    /\b(?:deadline|due\s+date|target\s+date|completion\s+date|launch\s+date|go[-\s]?live)\b/gi,
...
export const SECURITY_PATTERNS = {
  ...
  securityLanguage:
    /\b(?:authentication|authorization|encryption|TLS|SSL|OAuth|RBAC|ABAC|PII|GDPR|HIPAA|SOC\s*2|PCI[-\s]?DSS)\b/gi,
  threatLanguage:
    /\b(?:threat|vulnerability|attack|exploit|injection|XSS|CSRF|malicious|adversar\w+)\b/gi,
...
export const DEPENDENCIES_PATTERNS = {
  ...
  upstreamLanguage:
    /\b(?:upstream|depends\s+on|requires|prerequisite|blocked\s+by|waiting\s+on)\b/gi,
...
export const DIAGRAM_PATTERNS = {
  ...
  diagramReference:
    /\b(?:see\s+(?:diagram|figure|chart)|(?:diagram|figure|chart)\s+\d+|as\s+shown\s+in|illustrated\s+in)\b/gi,
  diagramType:
    /\b(?:sequence\s+diagram|class\s+diagram|flow\s*chart|UML|C4\s+model|ERD|data\s+flow|state\s+machine)\b/gi,
...
export const OBSERVABILITY_PATTERNS = {
  ...
  metricsLanguage:
    /\b(?:SLO|SLA|SLI|latency|throughput|error\s+rate|availability|uptime|p99|p95|p50)\b/gi,
  monitoringLanguage:
    /\b(?:dashboard|alert|notification|on[-\s]?call|incident|runbook|playbook|grafana|datadog|prometheus)\b/gi,
  tracingLanguage:
    /\b(?:tracing|distributed\s+tracing|trace|span|correlation\s+ID|request\s+ID|opentelemetry|jaeger)\b/gi,
};
