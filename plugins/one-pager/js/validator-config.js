...
export const REQUIRED_SECTIONS = [
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(problem|challenge|pain.?point|context)/im,
    name: 'Problem/Challenge',
    weight: 2,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(solution|proposal|approach|recommendation)/im,
    name: 'Solution/Proposal',
    weight: 2,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(goal|objective|benefit|outcome)/im,
    name: 'Goals/Benefits',
    weight: 2,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(scope|in.scope|out.of.scope|boundary|boundaries)/im,
    name: 'Scope Definition',
    weight: 2,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(success|metric|kpi|measure|success.metric)/im,
    name: 'Success Metrics',
    weight: 1,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(stakeholder|team|owner|raci|responsible)/im,
    name: 'Stakeholders/Team',
    weight: 1,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(timeline|milestone|phase|schedule|roadmap)/im,
    name: 'Timeline/Milestones',
    weight: 1,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(investment|effort|resource|cost|budget)/im,
    name: 'Investment/Resources',
    weight: 2,
  },
  {
    pattern: /^(#+\s*)?(\d+\.?\d*\.?\s*)?(risk|assumption|mitigation|dependency|dependencies)/im,
    name: 'Risks/Assumptions',
    weight: 1,
  },
  {
    pattern:
      /^(#+\s*)?(\d+\.?\d*\.?\s*)?(cost.of.doing.nothing|cost.of.inaction|why.now|urgency)/im,
    name: 'Cost of Doing Nothing',
    weight: 2,
  },
  {
    pattern:
      /^(#+\s*)?(\d+\.?\d*\.?\s*)?(decision|ask|request|approval|next.?step|action.?needed|conclusion)/im,
...
export const PROBLEM_PATTERNS = {
  ...
  problemLanguage:
    /\b(problem|challenge|pain.?point|issue|struggle|difficult|frustrat|current.?state|today|existing)\b/gi,
  costOfInaction:
    /\b(cost|impact|consequence|risk|without|if.?not|delay|postpone|inaction|doing.?nothing|status.?quo)\b/gi,
  quantified:
...
export const SOLUTION_PATTERNS = {
  ...
  solutionLanguage:
    /\b(solution|approach|proposal|implement|build|create|develop|enable|provide|deliver)\b/gi,
  ...
  avoidImplementation:
    /\b(code|function|class|method|api|database|sql|algorithm|library|framework)\b/gi,
};
...
export const SCOPE_PATTERNS = {
  ...
  outOfScope:
    /\b(out.of.scope|not.included|excluded|we.will.not|won't|outside.scope|future|phase.2|post.mvp|not.in.v1)\b/gi,
...
export const METRICS_PATTERNS = {
  ...
  quantified:
    /\d+\s*(%|million|thousand|hour|day|week|month|year|\$|dollar|user|customer|transaction|request|response)/gi,
...
export const STAKEHOLDER_PATTERNS = {
  stakeholderSection:
    /^(#+\s*)?(\d+\.?\d*\.?\s*)?(stakeholder|team|owner|raci|responsible|responsible.accountable)/im,
  stakeholderLanguage:
    /\b(stakeholder|owner|lead|team|responsible|accountable|raci|sponsor|approver)\b/gi,
  stakeholderConcerns:
    /\b(finance|fp&a|fp.?&.?a|financial.planning|hr|people.?team|people.?ops|legal|compliance|equity|liability|approval|sign.?off|cfo|cto|ceo|vp|director)\b/gi,
...
export const ALTERNATIVES_PATTERNS = {
  alternativesSection:
    /^(#+\s*)?(\d+\.?\d*\.?\s*)?(alternative|option|why.?this|comparison|considered)/im,
  alternativesLanguage:
    /\b(alternative|option|instead|compared|versus|vs\.?|over|rather.than|considered|rejected|evaluated|chose|chosen)\b/gi,
...
export const URGENCY_PATTERNS = {
  ...
  urgencyLanguage:
    /\b(why.?now|urgent|window|opportunity|deadline|expire|time.?sensitive|competitive|first.?mover|market.?timing)\b/gi,
  timePressure:
    /\b(before|by|within|deadline|end.?of|q[1-4]|eoy|eoq|this.?quarter|this.?month|this.?year)\b/gi,
};
...
export const DECISION_NEEDED_PATTERNS = {
  decisionSection:
    /^(#+\s*)?(\d+\.?\d*\.?\s*)?(decision|ask|request|approval|next.?step|action.?needed)/im,
  decisionLanguage:
    /\b(decision|approve|approval|authorize|sign.?off|green.?light|go.ahead|proceed|fund|allocate|commit)\b/gi,
  explicitAsk:
    /\b(please\s+approve|requesting|we\s+ask|we\s+need|approve\s+this|decision\s+needed)\b/gi,
};
...
export const VAGUE_QUANTIFIER_PATTERNS = {
  ...
  vagueTerms:
    /\b(tbd|to\s+be\s+determined|some\s+amount|various|several|many|few|numerous|multiple|significant|substantial|considerable|approximately|around|about|roughly|could\s+be\s+anywhere|depending\s+on|hard\s+to\s+quantify)\b/gi,
...
export const ALTERNATIVES_QUALITY_PATTERNS = {
  ...
  rejectionRationale:
    /\b(rejected|not\s+chosen|ruled\s+out|unacceptable|insufficient|too\s+(?:expensive|slow|risky|complex)|doesn't\s+(?:address|solve)|won't\s+work)\b/gi,
  ...
  numberedAlternatives:
    /^(?:\d+\.|[-*])\s*\*?\*?(?:option|alternative|approach)\s*\d?\s*\*?\*?[:)]/gim,
