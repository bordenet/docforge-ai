...
export const TITLE_AUTO_DETECT = {
  howTo:
    /^(create|connect|set\s+up|configure|add|enable|disable|migrate|install|update|upgrade|export|import|generate)\b/i,
  troubleshooting:
    /^(why|error|cannot|can't|not\s+working|failed|failing|unable\s+to|fix|resolve)\b/i,
};
...
export const TITLE_PATTERNS = {
  ...
  actionable:
    /\b(fail|error|broken|can't|cannot|not\s+working|rejected|denied|invalid|missing|blocked|crash|loop|redirect|timeout|403|404|500|502|503)\b/i,
...
export const VAGUE_ADVERBS = ['properly', 'correctly', 'appropriately'];

// Passive voice in steps
export const PASSIVE_VOICE_PATTERN =
  /\b(should be|can be|may be|will be|could be)\s+(configured|set|updated|enabled|disabled|changed|modified)\b/gi;

// Future tense deferral (ServiceNow anti-pattern)
export const FUTURE_TENSE_PATTERN =
  /\b(you will need to|you'll need to|the system will|it will be)\b/gi;
...
export const UI_PATH_MULTILEVEL_PATTERN =
  /[A-Z][a-zA-Z ]{2,30}(?:\s*[→>]\s*[A-Z][a-zA-Z ]{2,30}){2,}/g;
...
export const CLI_COMMAND_PATTERN =
  /`\s*(?:\$\s+)?[a-z][\w./-]+(?:\s+[\w./=@:-]+)+\s*`|(?:^|\n)\s*(?:\$[^\S\n]+)?[a-z][\w./-]+(?:[^\S\n]+[\w./=@:-]+)*[^\S\n]+--?[\w-]+/gm;
...
export const BRANCH_CONDITION_PATTERN =
  /\b(if\s+.{5,60}?[,;]\s+(?:then\s+)?\w|otherwise,?\s+|unless\s+\w|if\s+this\s+(?:doesn't|does not|fails))/gi;

// KCS reproducible verbs at step-start
// Accept numbered OR bulleted steps, and an optional leading **bold**
export const REPRODUCIBLE_VERB_PATTERN =
  /^\s*(?:[-*]\s+|\d+\.\s+)?(?:\*\*)?(click|open|set|restart|deploy|run|confirm|navigate|select|enter|type|press|upload|download|copy|paste|enable|disable|toggle)\b/gim;

// ── VERIFICATION ──────────────────────────────────────────────────────────────

export const VERIFICATION_SPECIFIC_PATTERN =
  /\b(you\s+should\s+see|expect(?:ed)?\s+(?:to\s+see|output|result|response)|look\s+for|success\s+(?:state|message|indicator)|response\s+code\s+\d{3}|returns?\s+\d{3}|confirm\s+(?:that\s+)?.{5,30}\s+(?:appears?|shows?|changes?|says?|reads?))\b/gi;

export const VERIFICATION_VAGUE_PATTERN =
  /\b(verify\s+(?:that\s+)?(?:it\s+)?(?:is\s+)?(?:working|works|correct)|test\s+the\s+(?:connection|integration)(?:\s+again)?|try\s+(?:it\s+)?again|check\s+if\s+it\s+works)\b/gi;

// ── ESCALATION (3-component model) ──────────────────────────────────────────

export const ESCALATION_PATTERNS = {
  trigger:
    /\b(escalate\s+(?:if|when)|contact\s+support\s+if|open\s+a?\s+ticket\s+if|if\s+.{5,40}\s+(?:persists?|continues?|occurs?\s+again|still|remains?))\b/gi,
  threshold:
    /\b(\d+\s+(?:retries?|attempts?|times?|users?|accounts?|minutes?|hours?)|more\s+than\s+\d+|after\s+(?:trying|attempting|retrying))\b/gi,
  evidence:
    /\b(include|collect|capture|provide|attach|send|share)\b.{5,80}\b(request\s+id|log|timestamp|error\s+message|screenshot|trace|transaction\s+id|correlation\s+id)\b/gi,
...
export const SECTION_PATTERNS = {
  ...
  escalation:
    /^#+\s*(escalat|if\s+(?:it\s+)?(?:still\s+)?fail|not\s+working|further\s+help|contact\s+support)/im,
  ...
  prerequisites:
    /^#+\s*(prerequisite|before\s+you\s+(?:start|begin)|requirements?|you\s+will\s+need)/im,
};

// ── PRECISION / TECHNICAL ACCURACY ───────────────────────────────────────────

export const CAUSAL_LANGUAGE_PATTERN =
  /\b(because|caused\s+by|due\s+to|results?\s+from|triggered\s+by|occurs?\s+when|stems?\s+from|root\s+cause)\b/gi;

export const ENVIRONMENT_SPECIFICITY_PATTERNS = {
  version: /\b(v\d+\.\d+|\d+\.\d+\.\d+|version\s+\d+|release\s+\d+)\b/gi,
  platform:
    /\b(windows|macos|linux|ubuntu|debian|chrome|safari|firefox|edge|ios|android|aws|azure|gcp|kubernetes|docker)\b/gi,
  integration:
    /\b(salesforce|okta|azure\s+ad|google|slack|github|jira|zendesk|stripe|twilio|hubspot)\b/gi,
};

export const BOUNDARY_PATTERNS =
  /\b(does\s+not\s+apply|not\s+applicable|this\s+article\s+does\s+not|outside\s+scope|if\s+you\s+are\s+(?:not|using))\b/gi;
...
export const VAGUE_QUALIFIER_MULTI_WORD = ['in no time', 'in a few clicks', 'without any issues'];
...
export const SELF_SERVICE_PATTERNS = {
  ...
  timeEstimate:
    /\b(estimated\s+time|this\s+takes|approximately|~\s*\d+\s*(?:min|minute|hour)|\d+[\s-]+minute\s+guide)\b/gi,
  dangling:
    /\b(see\s+(?:the\s+)?(?:docs|documentation|guide|article)(?!\s+(?:at|here|linked|below|above)))\b/gi,
};
